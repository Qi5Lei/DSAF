...
class BaseRunner(object):
    ...
    def __init__(
        ...
        if self.is_pseudo:
            if self.label_generator is None:
                if self.model_dsbn is None:
                    self.label_generator = LabelGenerator(self.cfg, self.model)
                else:
                    self.label_generator = LabelGenerator(self.cfg, self.model_dsbn)
    ...
    def train_step(self, iter, batch):
        ...
        #results = self.model(inputs, ts_epoch = self._epoch)
    ...
    def save(self, mAP=None, path=None):
        if mAP is not None:
