...
def test_reid(
    ...
    if cfg.TEST.fusion == False:
        ...
        feat_list = []
        for idx in range(len(cfg.TRAIN.datasets.keys())):
        ...
        features = torch.mean(torch.stack(feat_list), 0)
...
def val_reid(
    ...
    if dataset_name is not None:
        ...
        if dataset_name in list(cfg.TRAIN.datasets.keys()):
            bn_idx = list(cfg.TRAIN.datasets.keys()).index(dataset_name)
            new_bn_idx = int((bn_idx+1)%len(list(cfg.TRAIN.datasets.keys())))
            switch_target_bn(model, new_bn_idx)
        else:
            warnings.warn(
                f"the domain of {dataset_name} does not exist before, "
                f"the performance may be bad."
