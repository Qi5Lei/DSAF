...
from openunreid.models.utils.dsbn_utils import switch_target_bn
...
class LabelGenerator(object):
    ...
    def __call__(self, epoch, cuda=True, memory_features=None, **kwargs):
        ...
        for idx, (data_loader, dataset) in enumerate(
            ...
            if memory_features is None:
                ...
                for model in self.models:
                    ########################################
                    switch_target_bn(model, idx)
                    #######################################
                    features, true_lables = extract_features(
                        model,
                        data_loader,
                        dataset,
                        cuda,
            ...
            ####################################################################
            ####################################################################
            # feats = all_features.numpy()
            # np.save('market_ms_feats.npy', feats)
            # np.save('market_labels.npy', dataset)
            ...
            all_true_labels.append(true_lables.tolist())
        ...
        if self.verbose:
            ...
            for label, true_lables, dn in zip(all_labels, all_true_labels, dataset_names):
                self.print_label_summary(epoch, label, true_lables, dn)

        return all_labels, all_centers

    def print_label_summary(self, epoch, pseudo_labels, true_labels, dataset_name):
        # statistics of clusters and un-clustered instances
        # print(pseudo_labels)
        # print(true_labels)
        new_pseudo_labels = copy.deepcopy(pseudo_labels)
        index2label = collections.defaultdict(int)
        init_count = len(pseudo_labels)
        for idx, label in enumerate(pseudo_labels):
            index2label[label] += 1
            if label == -1:
                new_pseudo_labels[idx] = init_count
        ...
        # fowlkes_mallows_score, adjusted_mutual_info_score, adjusted_rand_score
        ####
        print(
            ...
            f"{adjusted_rand_score(true_labels, new_pseudo_labels)} ARI\n"
            f"{adjusted_mutual_info_score(true_labels, new_pseudo_labels)} AMI\n"
            f"{fowlkes_mallows_score(true_labels, new_pseudo_labels)} FMI\n"
        )
