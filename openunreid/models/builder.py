...
from .utils.dsbn_utils import convert_dsbn, convert_ibn_share_in_affine_for_dsbn
...
class ReIDBaseModel(nn.Module):
    ...
    def forward(self, x, ts_epoch=None):
        batch_size = x.size(0)
        results = {}
        if ts_epoch == None:
            x = self.backbone(x)
        else:
            x = self.backbone(x, ts_epoch)
...
def convert_train_to_eval(model):
    """
    convert all bn layers in the model to domain-specific bn layers
    """
    for _, (child_name, child) in enumerate(model.named_children()):

        # if isinstance(child, ADA_NORM) or isinstance(child, ADA_NORM1d):
        #     # BN2d -> DSBN2d
...
def build_model(
    ...
    if (num_domains > 1) and cfg.MODEL.dsbn:
        ...
        # if cfg.MODEL.ibn_f == 22:  #####DSON#######
        #     convert_dson(model, num_domains, target_domain_idx)
        # else:
        convert_dsbn(model, num_domains, target_domain_idx)
        if cfg.MODEL.ibn_f == 20:  #####ours_method#######
            convert_ibn_share_in_affine_for_dsbn(model, num_domains, target_domain_idx, cfg.MODEL.ibn_layers)
    ...
    if cfg.MODEL.sync_bn and dist:
        if cfg.TRAIN.LOADER.samples_per_gpu < cfg.MODEL.samples_per_bn:
            ...
            if cfg.MODEL.pre_trained is not None:
                convert_sync_bn(model_dsbn, dist_groups)
