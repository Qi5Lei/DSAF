...
def main():
    ...
    if cfg.MODEL.pre_trained is not None:
        model, model_dsbn = build_model(cfg, num_classes, init=cfg.MODEL.source_pretrained)
        model_dsbn.cuda()
    else:
        model = build_model(cfg, num_classes, init=cfg.MODEL.source_pretrained)
    model.cuda()
    ###########################################
    # path ='/home/qilei/code/OpenUnReID/exp/strong_baseline/55_un_duke_msmt_layer1234/model_best.pth'
    ...
    if dist:
        if cfg.MODEL.pre_trained is not None:
            model_dsbn = torch.nn.parallel.DistributedDataParallel(
                model_dsbn,
                device_ids=[cfg.gpu],
        ...
        if cfg.MODEL.pre_trained is not None:
            model_dsbn = torch.nn.DataParallel(model_dsbn)
    ###switch_target_bn(model, 2)
    # build optimizer
